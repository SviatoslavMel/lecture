 ...
 * @param {Number} id  оголошення */
const getMarka = (id) => pool.query('SELECT * FROM lecture.marka WHERE id=?', [id])
    .then(([[data]]) => data);
...
getAuto(1)
    .then((autoData) => Promise.all([
            getMarka(autoData.marka_id),
            getModel(autoData.model_id)
        ])
            .then(([marka, model]) => printAuto({...autoData, marka, model}))
    )
    .catch((e) => console.error(e))
    .finally(() => console.log('I`m ready!'))
